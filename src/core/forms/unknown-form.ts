...
export class UnknownForm extends AbstractForm<UnknownPage> {
  readonly formType = 'UNKNOWN' as const;
  readonly name = 'Unbekanntes Formular';

  // Patterns for extraction
  private readonly FORM_NUMBER_PATTERN = /(?:Form\.-Nr\.|Formular-Nr\.|F\.-Nr\.)\s*:?\s*([A-Z0-9]+)/i;
  private readonly DATE_PATTERN = /(Januar|Februar|März|April|Mai|Juni|Juli|August|September|Oktober|November|Dezember)\s*(\d{4})/i;

  extractMetadata(text: string, pageIndex: number, rawText: string): UnknownPage {
    return {
  ...
  isFirstPage(_text: string): boolean {
    return true;  // Can't determine, assume true
  }

  hasPersonnelNumbers(): boolean {
    return false;  // Unknown forms don't have extractable personnel numbers
  }
